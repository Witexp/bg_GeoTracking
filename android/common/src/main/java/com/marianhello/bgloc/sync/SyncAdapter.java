...
public class SyncAdapter extends AbstractThreadedSyncAdapter implements HttpPostService.UploadingProgressListener {
    ...
    public void onPerformSync(
        ...
        boolean hasHttpTemplateRoot = config.hasHttpTemplateRoot();

        File file = null;
        try {
            if (hasHttpTemplateRoot){
                file = batchManager.createBatch(batchStartMillis, syncThreshold, config.getTemplate(), config.getHttpTemplateRoot());
            } else {
