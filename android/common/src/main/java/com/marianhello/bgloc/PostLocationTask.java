...
public class PostLocationTask {
    ...
    public void add(final BackgroundLocation location) {
        ...
        try {
            mExecutor.execute(new Runnable() {
                ...
                public void run() {
                    post(location);
                    logger.warn("PostLocationTask ---  post(location) ---.");
                }
            });
        } catch (RejectedExecutionException ex) {
            mLocationDAO.updateLocationForSync(locationId);
            logger.warn("PostLocationTask --- updateLocationForSync ----.");
        }
    }
    ...
    private boolean postLocation(BackgroundLocation location) {
        ...
        String httpRoot = mConfig.getHttpTemplateRoot();
        logger.debug(" --- httpRoot --- httpTemplateRoot: {} ", httpRoot.toString());
        try {
            jsonLocations.put(mConfig.getTemplate().locationToJson(location));
            //jsonObjectLocation.put("data",jsonLocations);
            jsonObjectLocation.put(httpRoot,jsonLocations);
        ...
        logger.debug("Posting json to url: {}", url.toString() );
        int responseCode;

        try {
            Boolean hasHttpRoot = mConfig.hasHttpTemplateRoot();
            //logger.debug(" --- hasHttpRoot --- ", mConfig.hasHttpTemplateRoot());
            if (hasHttpRoot){
                logger.debug(" --- hasHttpRoot ---  hasHttpTemplateRoot: {}", hasHttpRoot.toString());
                responseCode = HttpPostService.postJSON(url, jsonObjectLocation, mConfig.getHttpHeaders());
            } else {
                logger.debug(" --- hasHttpRoot ---  jsonLocations");
